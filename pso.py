...
def next(bot, glo, loc):
    x = int(bot[0]+np.random.uniform(0,1)*g*(glo[0]- bot[0]) + np.random.uniform(0,1)*l*(loc[0] - bot[0])) + 1
    y = int(bot[1]+np.random.uniform(0,1)*g*(glo[1]- bot[1]) + np.random.uniform(0,1)*l*(loc[1] - bot[1])) + 1
    x = np.clip(x,bot[0]-3,bot[0]+3)
    y = np.clip(y,bot[1]-3,bot[1]+3)
    if x > row -1:
        x = row -1
    if y > column -1:
        y = column -1
